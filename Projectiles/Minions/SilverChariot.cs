...
namespace cool_jojo_stands.Projectiles.Minions
{
    public class SilverChariot : Stand
    {
        ...
        public override void SelectFrame()
        {
            ...
            if (attacking)
            {
                if (pl.StandPornoleffSetBonus > 0 || true)
                {
                    if (Projectile.frameCounter >= 4)
                    {
                        Projectile.frame = 8 + (Projectile.frame + 1) % 2;
                        Projectile.frameCounter = 0;
                    }
                }
                else
                {
                    if (Projectile.frame == 7 && Projectile.frameCounter >= 7)
                    {
                        Projectile.frameCounter = 0;
                        Projectile.frame = 4 + Main.rand.Next(3);
                    }
                    else if (Projectile.frame != 7 && Projectile.frameCounter >= 6)
                    {
                        Projectile.frameCounter = 0;
                        Projectile.frame = 7;
                    }
